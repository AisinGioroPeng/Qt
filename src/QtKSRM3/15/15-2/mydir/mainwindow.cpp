    ...
    ui(new Ui::MainWindow)
{
    ...
    // 将监视器的信号和自定义的槽进行关联        注意:当文件被重命名或者移除后，或者当目录被移除后，QFileSystemWatcher都会停止监视
    connect(&myWatcher, &QFileSystemWatcher::directoryChanged,this, &MainWindow::showMessage); //当目录或者它的内容被修改或者移除后，会发射directoryChanged()信号
    connect(&myWatcher, &QFileSystemWatcher::fileChanged,this, &MainWindow::showMessage); //文件被修改、重命名或者移除后，会发射fileChanged()信号
    // 显示出当前目录下的所有.h文件
    QDir myDir(QDir::currentPath());
    myDir.setNameFilters(QStringList("*.h"));
    ui->listWidget->addItem(myDir.absolutePath() + tr("目录下的.h文件有："));
    ui->listWidget->addItems(myDir.entryList()); //返回目录中所有文件和目录的名称列表
    // 创建目录，并将其加入到监视器中
    myDir.mkdir("mydir");
    myDir.cd("mydir");
